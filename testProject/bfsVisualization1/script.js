...
function bfs() {
    if (!isVisualizing || queue.length === 0) return;
    ...
    if (currentNode === endNode) {
        visualizePath();  // Visualize path after reaching the end node
        return;
    }
    ...
    const timeoutId = setTimeout(bfs, 0); // Delay of 5ms between each step
